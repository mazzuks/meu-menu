...
export function RecipeCard({
  recipe,
  variant = 'vertical',
  fill = false,           // <- use fill na grade (/receitas). no carrossel fica false (largura fixa)
  className = '',
}: {
  recipe: Recipe
        ...
        <div className="flex">
          {img && (
            <div className="w-28 h-28 overflow-hidden flex-shrink-0">
              <img src={img} alt={recipe.title} className="w-full h-full object-cover block" />
            </div>
          )}
          <div className="p-3 flex-1">
            ...
            <div className="flex items-center gap-3 text-[11px] text-gray-600 mt-2">
              {recipe.prepTime != null && (<span className="flex items-center gap-1"><Clock className="w-3 h-3" />{recipe.prepTime} min</span>)}
              {recipe.difficulty && (<span className="flex items-center gap-1"><ChefHat className="w-3 h-3" />{recipe.difficulty}</span>)}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div
      className={[
        ...
        fill ? 'w-full' : 'flex-none w-44 sm:w-48', // <- largura fixa no carrossel
        className,
      ].join(' ')}
    >
      <Link href={href} className="absolute inset-0 z-10" aria-label={recipe.title} />
      {/* Mantém proporção e evita “colapsar” */}
      <div className="aspect-[4/3] w-full overflow-hidden">
        {img && <img src={img} alt={recipe.title} className="w-full h-full object-cover block" />}
      </div>
      <div className="p-3 space-y-1">
        <h3 className="font-medium leading-tight line-clamp-1">{recipe.title}</h3>
