...
export function RecipeCard({
  ...
  const title = recipe.title || recipe.name || 'Receita'

  if (variant === 'horizontal') {
    return (
      <Link href={href} className={`block bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 ${className}`} aria-label={title}>
        <div className="flex">
          {img && (
